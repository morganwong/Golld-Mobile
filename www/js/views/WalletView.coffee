class WalletView extends BaseView
  constructor: ()->
    ...
    @template = @tmpl
    # @evtHandler = new WalletEvtHandler(@)
...
#    , 500)
    ...
    address = AppProps.get "tgwaddress"
    if address is null
      ...
      if currentTokens.golldvouchers?
        ...
        for k of currentTokens.golldvouchers
          ...
          if currentTokens.golldvouchers[k]
            ...
            $('#tokenslist').html k + ": <span id='" + k + "_amountspan'>" + bal + "<i style='float: right;' class='icon ion-android-send' id='send_" + k + "'>Send</i></span><br>"
  ...
  tmpl: (data) ->
    """
      <div class='bar bar-header'>
        <h1 class='title' id='balanceh1'></h1>
      </div>

      <!-- <div class='loadingoverlay'>
        <img style='margin-top: 40%' src='img/loading.svg'/>
        <p style='font-style: italic' id='loadingmsg'></p>
      </div> -->
      ...
      <div id='walletcontent' class='splash headerabove'>
        ...
        <ul class="list">
          <li class="item" style='line-height: 160%'>
            ...
            <p id='address'></p>
            <span class='walletaddressicons'>
              <i class='icon ion-ios-copy' id='icopy'></i>
              <i class='icon ion-tgqrcustom' id="iqr" style='width: 12px; height: auto;'></i>
              <i class='icon ion-forward' id='iforward'></i>
            </span>
          </li>
          <li class='item' style='line-height: 160%'>
            <span>Tokens</span>
            ...
            <button id="addtokenbutton" class='button button-assertive' style='float: right;'>
              Add Token To Wallet
            </button>
          </li>
        </ul>

        <div id="qrdiv" style='position: absolute; display: none; top: 0px; left: 0px; width: 100%; height: 100%; z-index: 99; text-align: center; background-color: white;'>
          <img id="qrimg" src='' style='margin-top: 40px; margin-left: 10%; margin-right: 10%; width: auto; height: auto;'/>
          <br><br><br>
          <p id='qrdivaddress'></p>
          <p>tap to close...</p>
        </div>
